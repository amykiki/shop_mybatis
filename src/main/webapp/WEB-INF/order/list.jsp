...
<body>
<c:url var="pageurl" value="/order.do?method=list&name=${param.name}&oid=${param.oid}&ostatus=${param.ostatus}"/>
<c:url var="actionurl" value="/order.do?name=${param.name}&oid=${param.oid}&ostatus=${param.ostatus}"/>
                ...
                <div class="order-body">
                    <div class="o-col1">
                        ...
                        <div class="o-cols o-col4 o-cols-last">
                            <c:if test="${oitems.status == ToPAID}">
                                <a href="${actionurl}&method=payOrder&orderid=${oitems.id}"><span>去付款</span></a>
                            </c:if>
                            <c:if test="${oitems.status == ToCONFIRMED}">
                                <a href="${actionurl}&method=confirmOrder&orderid=${oitems.id}">确认收货</a>
                            </c:if>
                            <c:if test="${oitems.status == FINISHED}">
                                <a href="${actionurl}&method=deleteOrder&orderid=${oitems.id}">删除订单</a>
                            </c:if>
                            <c:if test="${oitems.status == CANCELED}">
                                <a href="${actionurl}&method=deleteOrder&orderid=${oitems.id}">删除订单</a>
                            </c:if>
